...
public class IntakeSubsystem extends SubsystemBase implements Sendable {
    ...
    public IntakeSubsystem() {
        Feed1 = new MotorIOSparkMax(11); //change canID later
        Feed2 = new MotorIOSparkMax(12); //change canID later
        ...
        Feed1_pid = new PIDController(kp, ki, kd);   // TODO decide if we need separate pids (ask henry)
    ...
    public void periodic() { // Update inputs, calculate, then set voltages every loop
        ...
        if (Intakestatus && !lastIntakestatus) { // If true (intake is on and shooter was off)
            resetPID();
        }

        if (!Intakestatus) {  // if true intake is on
            calculate();
